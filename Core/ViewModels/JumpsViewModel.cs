...
namespace BaseLogApp.Core.ViewModels
{
    public enum JumpSaveErrorCode
    {
        ...
        UnknownJumpType,
        UnknownRig,
        ConflictRequiresShiftConfirmation,
        ConflictWithoutShiftSupport,
        ShiftFailed,
        SaveFailed
    }

    public sealed class JumpSaveResult
    {
        public bool Success { get; init; }
        public bool RequiresConfirmation { get; init; }
        public JumpSaveErrorCode ErrorCode { get; init; }
    ...
    public class JumpsViewModel : INotifyPropertyChanged
    {
        ...
        public int TotalJumps
        {
            ...
            private set
            {
                if (_totalJumps == value) return;
                _totalJumps = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(FilterCountText));
            }
        }
        public string FilterCountText => $"{Count}/{TotalJumps}";
        ...
        private DbProfile _currentProfile = DbProfile.Legacy;
        ...
        public string CurrentProfileLabel => $"DB: {Path.GetFileName(GetCurrentDbPath())}";
        ...
        {
            _reader = reader;
            _reader.SetDbProfile(DbProfile.Legacy);
        }
        ...
        public async Task<bool> SetCustomDbPathAsync(string dbPath)
        {
            var ok = _reader.SetCustomDbPath(dbPath);
            if (!ok)
                return false;
        ...
        public async Task<JumpSaveResult> SaveJumpWithValidationAsync(JumpListItem jump, bool allowShiftOnConflict)
        {
            if (jump.NumeroSalto <= 0)
            {
                return new JumpSaveResult
                {
                    Success = false,
                    ErrorCode = JumpSaveErrorCode.InvalidJumpNumber,
                    UserMessage = "Inserisci un numero salto valido."
                };
            }

            var objectName = (jump.Oggetto ?? string.Empty).Trim();
            if (!string.IsNullOrWhiteSpace(objectName))
            {
                var validObjects = await _reader.GetObjectNamesAsync();
                if (!validObjects.Any(x => string.Equals(x.Trim(), objectName, StringComparison.OrdinalIgnoreCase)))
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        ErrorCode = JumpSaveErrorCode.UnknownObject,
                        UserMessage = $"Object '{objectName}' non presente nel database."
                    };
                }
            }

            var jumpType = (jump.TipoSalto ?? string.Empty).Trim();
            if (!string.IsNullOrWhiteSpace(jumpType))
            {
                var validJumpTypes = await _reader.GetJumpTypeNamesAsync();
                if (!validJumpTypes.Any(x => string.Equals(x.Trim(), jumpType, StringComparison.OrdinalIgnoreCase)))
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        ErrorCode = JumpSaveErrorCode.UnknownJumpType,
                        UserMessage = $"Tipo salto '{jumpType}' non presente nel database."
                    };
                }
            }

            if (jump.RigNames.Count > 0)
            {
                var validRigs = await _reader.GetRigNamesAsync();
                var invalidRig = jump.RigNames
                    .FirstOrDefault(r => !validRigs.Any(v => string.Equals(v.Trim(), r.Trim(), StringComparison.OrdinalIgnoreCase)));
                if (!string.IsNullOrWhiteSpace(invalidRig))
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        ErrorCode = JumpSaveErrorCode.UnknownRig,
                        UserMessage = $"Rig '{invalidRig}' non presente nel database."
                    };
                }
            }

            string? notice = null;
            var maxAllowed = GetMaxAllowedJumpNumber(jump.IsEdit ? jump.Id : null);
            if (jump.NumeroSalto > maxAllowed)
            {
                jump.NumeroSalto = maxAllowed;
                notice = $"Numero troppo alto: impostato automaticamente a {maxAllowed} (ultimo + 1).";

                AppLog.Warn(
                    GetCurrentDbPath() + ".log",
                    LogCategories.NumberShift,
                    nameof(JumpsViewModel),
                    nameof(SaveJumpWithValidationAsync),
                    "Jump number adjusted to max allowed.",
                    details: $"maxAllowed={maxAllowed};isEdit={jump.IsEdit};jumpId={jump.Id}");
            }

            var hasConflict = HasJumpNumberConflict(jump.NumeroSalto, jump.IsEdit ? jump.Id : null);
            if (hasConflict)
            {
                var canShift = await SupportsJumpNumberShiftAsync();
                if (!canShift)
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        ErrorCode = JumpSaveErrorCode.ConflictWithoutShiftSupport,
                        UserMessage = "Numero gia esistente. In questo DB non e possibile rinumerare automaticamente.",
                        Notice = notice
                    };
                }

                if (!allowShiftOnConflict)
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        RequiresConfirmation = true,
                        ErrorCode = JumpSaveErrorCode.ConflictRequiresShiftConfirmation,
                        UserMessage = $"Il salto #{jump.NumeroSalto} esiste gia. Vuoi spostare in avanti i salti successivi?",
                        Notice = notice
                    };
                }

                var shifted = await ShiftJumpNumbersUpFromAsync(jump.NumeroSalto, jump.IsEdit ? jump.Id : null);
                if (!shifted)
                {
                    return new JumpSaveResult
                    {
                        Success = false,
                        ErrorCode = JumpSaveErrorCode.ShiftFailed,
                        UserMessage = "Impossibile rinumerare i salti esistenti.",
                        Notice = notice
                    };
                }
            }

            var saved = await SaveJumpAsync(jump);
            if (!saved)
            {
                return new JumpSaveResult
                {
                    Success = false,
                    ErrorCode = JumpSaveErrorCode.SaveFailed,
                    UserMessage = "Impossibile salvare il salto nel database.",
                    Notice = notice
                };
            }

            jump.NewPhotoBytes = null;
            return new JumpSaveResult
            {
                Success = true,
                ErrorCode = JumpSaveErrorCode.None,
                UserMessage = "Salto salvato.",
                Notice = notice
            };
        }
        ...
        public async Task<bool> AddObjectAsync(string name, string? objectType, string? description, string? coordinatesText, string? region, string? heightMeters, byte[]? photoBytes)
            => await _reader.AddObjectAsync(name, objectType, description, coordinatesText, region, heightMeters, photoBytes);
        ...
        public Task<bool> AddDeploymentTypeAsync(string name, string? notes)
            => _reader.AddDeploymentTypeAsync(name, notes);

        public Task<bool> AddSliderTypeAsync(string name, string? notes)
            => _reader.AddSliderTypeAsync(name, notes);

        public Task<bool> AddPilotChuteTypeAsync(string name, string? notes)
            => _reader.AddPilotChuteTypeAsync(name, notes);

        public Task<bool> AddBrakeSettingAsync(string name, string? notes)
            => _reader.AddBrakeSettingAsync(name, notes);

        public Task<bool> UpdateObjectAsync(int id, string name, string? objectType, string? description, string? coordinatesText, string? region, string? heightMeters, byte[]? photoBytes)
            => _reader.UpdateObjectAsync(id, name, objectType, description, coordinatesText, region, heightMeters, photoBytes);
