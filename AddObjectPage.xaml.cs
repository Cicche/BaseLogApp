...
public partial class AddObjectPage : ContentPage
{
    ...
    private void FillFromObject(ObjectCatalogItem item)
    {
        ...
        ObjectRegionEntry.Text = item.Region;
    ...
    private async void OnOpenMapClicked(object sender, EventArgs e)
    {
        ...
        else if (!string.IsNullOrWhiteSpace(ObjectRegionEntry.Text))
            await Launcher.OpenAsync($"https://www.google.com/maps/search/?api=1&query={Uri.EscapeDataString(ObjectRegionEntry.Text)}");
        else
    ...
    private async void OnSaveClicked(object sender, EventArgs e)
    {
        var ok = _editing is null
            ? await _vm.AddObjectAsync(ObjectNameEntry.Text ?? string.Empty, ObjectTypeEntry.Text, ObjectDescriptionEntry.Text, ObjectPositionEntry.Text, ObjectRegionEntry.Text, ObjectHeightEntry.Text, _objectPhotoBytes)
            : await _vm.UpdateObjectAsync(_editing.Id, ObjectNameEntry.Text ?? string.Empty, ObjectTypeEntry.Text, ObjectDescriptionEntry.Text, ObjectPositionEntry.Text, ObjectRegionEntry.Text, ObjectHeightEntry.Text, _objectPhotoBytes);
